...
using GameArchitectureExample.GamePlayScreenBackgrounds.GamePlayScreen3;
...
namespace GameArchitectureExample.GamePlayScreens
{
    public class GamePlayScreen4 : GameScreen
    {
        ...
        private readonly InputAction ContinueNextLevel = new InputAction(
                new[] { Buttons.Start, Buttons.A },
        ...
        private World world;
        private Vector2 initialPosition = new Vector2(500, 400);
        ...
        BackgroundMidgroundForeground _background;
        #endregion
        ...
        public GamePlayScreen4()
        {
            ...
            Bat = new Enemy1(initialPosition);
            GameConstants.ChangeGameScreen(this);
            _background = new BackgroundMidgroundForeground();
        }
        ...
        {
            if (protagonist.ProtagonistBody.Position.X > 1200)
            {
                win = true;
            }
        }
        ...
        public override void HandleInput(GameTime gameTime, InputState input)
        {
            ...
            if (win)
            {
                if (ContinueNextLevel.Occurred(input, ControllingPlayer, out player))
                {
                    LoadingScreen.Load(ScreenManager, false, null, new BackgroundScreen(), new MainMenuScreen());
                }
                return;
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            float playerX = MathHelper.Clamp(protagonist.ProtagonistBody.Position.X, 400, 800);
            offset = 400 - playerX;
            ...
            Bat.Draw(gameTime, ScreenManager.SpriteBatch);

            protagonist.Draw(gameTime, ScreenManager.SpriteBatch);
            ScreenManager.SpriteBatch.End();

            ScreenManager.SpriteBatch.Begin(transformMatrix: transform);
            if (timer < 0) { ScreenManager.SpriteBatch.DrawString(_gameFont, "You     Lost", new Vector2(protagonist.ProtagonistBody.Position.X - 100, 150), Color.White); }
            if (timer < 0) { ScreenManager.SpriteBatch.DrawString(_gameFont, "Press     R     to     restart", new Vector2(protagonist.ProtagonistBody.Position.X - 100, 200), Color.White); }
            ScreenManager.SpriteBatch.End();
